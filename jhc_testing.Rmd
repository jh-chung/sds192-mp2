...
Pplot_z <- function(state_put) {
    ...
    theme(plot.title = element_text(size = 12),
          legend.position = "none") +
    labs(x = "Party", y = "Total Dollars") + 
    ggtitle(paste(which(c(PlistDataset) == cand_office_state), ". " , cand_office_state, sep = "")) 
}
# Maybe I want to filer by positive states
  lapply(PlistDataset , FUN = Pplot_z)
  
  # THese below do not have the titles
Pgraph_z <- lapply(PlistDataset , FUN = contributions_plot)
do.call("grid.arrange" , c(Pgraph_z , ncol = 5))
